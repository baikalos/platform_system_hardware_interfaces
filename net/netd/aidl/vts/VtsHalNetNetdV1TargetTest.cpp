...
class NetdAidlTest : public ::testing::TestWithParam<std::string> {
   public:
    ...
    virtual void SetUp() override {
        ...
        auto ret = netd->createOemNetwork(&oemNetwork, &status);
    ...
    virtual void TearDown() override {
        INetd::StatusCode status;
        netd->destroyOemNetwork(mNetHandle, &status);
        ASSERT_EQ(INetd::StatusCode::OK, status);
    }
    ...
    void expectAddRoute(INetd::StatusCode expectedStatus, net_handle_t handle, const char* iface,
        ...
        INetd::StatusCode retStatus;
        auto ret = netd->addRouteToOemNetwork(handle, iface, destination, nexthop, &retStatus);
        EXPECT_TRUE(ret.isOk());
        EXPECT_EQ(expectedStatus, retStatus);
    }
    ...
    void expectRemoveRoute(INetd::StatusCode expectedStatus, net_handle_t handle, const char* iface,
                           const char* destination, const char* nexthop) {
        INetd::StatusCode retStatus;
        auto ret = netd->removeRouteFromOemNetwork(handle, iface, destination, nexthop, &retStatus);
        EXPECT_TRUE(ret.isOk());
        EXPECT_EQ(expectedStatus, retStatus);
    }
...
TEST_P(NetdAidlTest, TestAddRemoveInterfaces) {
    ...
    INetd::StatusCode retStatus;
    auto ret = netd->addInterfaceToOemNetwork(mNetHandle, sIfaceName, &retStatus);
    EXPECT_TRUE(ret.isOk());
    ...
    ret = netd->addInterfaceToOemNetwork(mNetHandle, sIfaceName, &retStatus);
    EXPECT_TRUE(ret.isOk());
    // TODO check status instead?
    // EXPECT_TRUE(retStatus.isOk());
    EXPECT_EQ(0, checkNetworkExists(mNetHandle));
    ...
    ret = netd->createOemNetwork(&oem, &retStatus);
    EXPECT_TRUE(ret.isOk());
    ...
    ret = netd->addInterfaceToOemNetwork(netHandle2, sIfaceName, &retStatus);
    EXPECT_TRUE(ret.isOk());
    EXPECT_EQ(INetd::StatusCode::UNKNOWN_ERROR, retStatus);

    // Removing the interface removes the rules.
    ret = netd->removeInterfaceFromOemNetwork(mNetHandle, sIfaceName, &retStatus);
    EXPECT_TRUE(ret.isOk());
    ...
    ret = netd->destroyOemNetwork(netHandle2, &retStatus);
    EXPECT_TRUE(ret.isOk());
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    ...
    ret = netd->addInterfaceToOemNetwork(INVALID_NET_HANDLE, sIfaceName, &retStatus);
    EXPECT_TRUE(ret.isOk());
    EXPECT_EQ(INetd::StatusCode::INVALID_ARGUMENTS, retStatus);
    ret = netd->removeInterfaceFromOemNetwork(INVALID_NET_HANDLE, sIfaceName, &retStatus);
    EXPECT_TRUE(ret.isOk());
    EXPECT_EQ(INetd::StatusCode::INVALID_ARGUMENTS, retStatus);
}
...
TEST_P(NetdAidlTest, TestForwarding) {
    INetd::StatusCode retStatus;
    auto ret = netd->addInterfaceToOemNetwork(mNetHandle, sTun1.name().c_str(), &retStatus);
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    ret = netd->addInterfaceToOemNetwork(mNetHandle, sTun2.name().c_str(), &retStatus);
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    ...
    ret = netd->setForwardingBetweenInterfaces(sTun1.name(), sTun2.name(), true, &retStatus);
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    ...
    ret = netd->setForwardingBetweenInterfaces(sTun1.name(), sTun2.name(), false, &retStatus);
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    EXPECT_EQ(2, countMatchingIpRules(regex1));

    ret = netd->setForwardingBetweenInterfaces(sTun2.name(), sTun1.name(), true, &retStatus);
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    EXPECT_EQ(2, countMatchingIpRules(regex1));
    ...
    ret = netd->setForwardingBetweenInterfaces(sTun2.name(), sTun1.name(), false, &retStatus);
    EXPECT_EQ(INetd::StatusCode::OK, retStatus);
    EXPECT_EQ(0, countMatchingIpRules(regex1));
    EXPECT_EQ(0, countMatchingIpRules(regex2));
