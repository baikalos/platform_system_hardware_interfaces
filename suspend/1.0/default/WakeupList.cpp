...
void WakeupList::evictIfFull() {
    if (mWakeups.size() == mCapacity) {
        deleteEntry(std::prev(mWakeups.end()));
        LOG(ERROR) << "WakeupList: Capacity met, consider adjusting capacity to "
                      "avoid stats eviction.";
    }
}

void WakeupList::insertEntry(WakeupInfo entry) {
    evictIfFull();
    mWakeups.emplace_front(std::move(entry));
    mLookupTable[entry.name] = mWakeups.begin();
}

void WakeupList::deleteEntry(std::list<WakeupInfo>::iterator entry) {
