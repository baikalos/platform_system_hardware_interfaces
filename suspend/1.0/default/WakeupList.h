...
class WakeupList {
   ...
   private:
    void evictIfFull();
    void insertEntry(WakeupInfo entry);
    void deleteEntry(std::list<WakeupInfo>::iterator entry);

    size_t mCapacity;
    mutable std::mutex mLock;
    std::list<WakeupInfo> mWakeups;  // Ordered by LRU
    std::unordered_map<std::string, std::list<WakeupInfo>::iterator> mLookupTable;
};
