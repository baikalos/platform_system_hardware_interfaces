...
static constexpr uint32_t kDefaultMaxSleepTimeMsec = 60000;
static constexpr uint32_t kDefaultBaseSleepTimeMsec = 100;
static constexpr double kDefaultSleepTimeScaleFactor = 2.0;
static constexpr uint32_t kDefaultBackoffThresholdCount = 0;
static constexpr uint32_t kDefaultShortSuspendThresholdMsec = 0;
...
int main() {
    ...
    LOG(ERROR) << "base=" << sleepTimeConfig.baseSleepTime.count();
    LOG(ERROR) << "max=" << sleepTimeConfig.maxSleepTime.count();
    LOG(ERROR) << "scale=" << sleepTimeConfig.sleepTimeScaleFactor;
    LOG(ERROR) << "back=" << sleepTimeConfig.backoffThreshold;
    LOG(ERROR) << "short=" << sleepTimeConfig.shortSuspendThreshold.count();
    LOG(ERROR) << "faileden=" << sleepTimeConfig.failedSuspendBackoffEnabled;
    LOG(ERROR) << "shorten=" << sleepTimeConfig.shortSuspendBackoffEnabled;
