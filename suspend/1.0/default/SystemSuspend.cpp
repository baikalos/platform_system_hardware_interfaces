...
int SystemSuspend::tryIncSuspendCounter(const string& name, int timeout_ms) {
    CHECK(timeout_ms > 0);
    if (!mAutosuspendLock.try_lock_for(std::chrono::milliseconds(timeout_ms)) {
        return -1;
    }
    if (mUseSuspendCounter) {
        mSuspendCounter++;
