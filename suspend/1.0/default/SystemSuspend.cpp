...
static std::vector<String16> readWakeupReasons(int fd) {
    std::vector<String16> wakeupReasons;
    string reasonline;
    ...
    while (ss.good()) {
        ...
        if (!reasonline.empty()) {
            wakeupReasons.push_back(String16(reasonline.c_str()));
        }
    }
...
void SystemSuspend::initAutosuspend() {
    std::thread autosuspendThread([this] {
        while (true) {
            ...
            const std::vector<String16> wakeupReasons = readWakeupReasons(mWakeupReasonsFd);
