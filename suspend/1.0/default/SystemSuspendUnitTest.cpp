   ...
   protected:
    static void registerTestService() {
        std::thread testService([] {
            ...
            wakeupReasonsFd =
                unique_fd(TEMP_FAILURE_RETRY(open(wakeupReasonsFile.path, O_CLOEXEC | O_RDWR)));
...
TEST_F(SystemSuspendTest, CallbackNotifyWakeupReason) {
    ...
    WriteStringToFd(wakeupReason1, wakeupReasonsFd);
    checkLoop(1);
    for (auto wakeupReason : impl.mWakeupReasons) {
        ASSERT_EQ(wakeupReason, referenceWakeupReason1);
    }

    WriteStringToFd(wakeupReason2, wakeupReasonsFd);
    checkLoop(1);
    for (auto wakeupReason : impl.mWakeupReasons) {
        ASSERT_EQ(wakeupReason, referenceWakeupReason2);
    }

    WriteStringToFd(wakeupReason3, wakeupReasonsFd);
    checkLoop(1);
    int i = 0;
    for (auto wakeupReason : impl.mWakeupReasons) {
    ...
    virtual void SetUp() override {
        ...
        suspendService =
            new SystemSuspend(unique_fd(-1) /* wakeupCountFd */, unique_fd(-1) /* stateFd */,
                              ...
                              unique_fd(dup(kernelWakelockStatsFd.get())), unique_fd(-1),
                              0ms /* baseSleepTime */, suspendControl);
    }
