...
// TimestampType is signed
inline static TimestampType abs(const TimestampType& value) {
...
TEST_F(SystemSuspendTest, GetWakeLockStats) {
    ...
    std::string fakeWlName = "FakeLock";
    {
        ...
        ASSERT_LT(abs(wlStats.begin()->activeSince - acquireTime), 1000);
        ASSERT_LT(abs(wlStats.begin()->lastChange - acquireTime), 1000);
    ...
    ASSERT_LT(abs(wlStats.begin()->activeSince - acquireTime), 1000);
    ASSERT_LT(abs(wlStats.begin()->lastChange - releaseTime), 1000);
}
