...
get_unlabeled_wakeup_paths() {
    for path in ${wakeup_paths[@]}; do
        # label the parent directory instead of each wakeupN directory
        dir_path="$(dirname $path)"
        if [ ! $(has_wakeup_attr $path) ]; then
            check_wakeup_dup $dir_path || unlabeled_paths+=( $dir_path )
        fi
    done
}
...
wakeup_paths+=( /sys/dev/test1/wakeup/wakeup3 )
get_unlabeled_wakeup_paths
