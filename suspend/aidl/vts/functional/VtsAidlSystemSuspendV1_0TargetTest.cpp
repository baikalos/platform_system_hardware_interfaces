...
class WakelockCallback : public BnWakelockCallback {
   public:
    Status notifyAcquired(void) override { return Status::ok(); };
    Status notifyReleased(void) override { return Status::ok(); };
};
...
class SystemSuspendTest : public testing::TestWithParam<std::string> {
   public:
    sp<ISuspendControlService> suspendControlService = nullptr;

    virtual void SetUp() override {
        LOG(ERROR) << __func__ << " BTAA hit";
...
TEST_P(SystemSuspendTest, RegisterWakeupCallback) {
...
TEST_P(SystemSuspendTest, RegisterMultipleWakeupCallback) {
    bool isRegistered = false;
...
TEST_P(SystemSuspendTest, RegisterWakelockCallback) {
    bool isRegistered = false;
...
TEST_P(SystemSuspendTest, RegisterSameWakelockCallbackWithSameName) {
    bool isRegistered = false;
...
TEST_P(SystemSuspendTest, RegisterSameWakelockCallbackWithDifferentNames) {
    bool isRegistered = false;
    sp<WakelockCallback> callback = new WakelockCallback();
...
TEST_P(SystemSuspendTest, RegisterDifferentWakelockCallbacksWithSameName) {
    bool isRegistered = false;
...
TEST_P(SystemSuspendTest, WakelockCallbackBinderDied) {
    bool isRegistered = false;
    sp<WakelockCallback> callback1 = new WakelockCallback();
    sp<WakelockCallback> callback2 = new WakelockCallback();
    ...
    callback1.clear();
    // Callback2 is untouched and should not get registered again, unless System Suspend service
    // crashes.
    isRegistered = false;
    ret = suspendControlService->registerWakelockCallback(callback2, kTestWakelockName,
                                                          &isRegistered);
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(SystemSuspendTest);
INSTANTIATE_TEST_SUITE_P(SystemSuspend, SystemSuspendTest,
                         testing::Values("android.system.suspend.ISuspendControlService"),
