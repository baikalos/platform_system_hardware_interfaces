...
interface IKeystoreService {
     ...
     * `nspace`, starting with `startEntry`.
     *
     ...
     *
     * Length of returned list: The returned list is at most `numEntries` long. If Keystore
     * estimates that the returned list would exceed the binder transaction size limit, it will
     * return a smaller number of entries.
     *
     * @param domain See `listEntries`
     *
     * @param nspace See `listEntries`
     *
     * @param startEntry The first entry to list from.
     *
     * @param numEntries The maximal number of entries to return.
     *
     * @return List of KeyDescriptors.
     */
    KeyDescriptor[] listEntriesBatched(in Domain domain, in long nspace,
            in int startEntry, in int numEntries);
