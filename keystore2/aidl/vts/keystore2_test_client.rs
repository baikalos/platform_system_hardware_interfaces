...
use std::ops::Deref;
use std::{collections::HashMap, sync::{Condvar, Mutex}};
...
impl OperationLimiter {
    ...
    fn get(&self, sec_level: SecurityLevel) -> OperationGuard {
        let max_ops =  self.max_ops.get(&sec_level).expect("Requested operation guard for unknown sec level.");
        let mut locked_current_ops = self.cond_var.wait_while(self.current_ops.lock().unwrap(), |ops| {
...
impl Drop for OperationGuard {
    fn drop(&mut self) {
        ...
        locked_current_ops.entry(self.0).and_modify(|e| { *e -= self.1});
...
mod tests {
    ...
    fn keystore2_pruning_test() {
        ...
        let sec_level =
            map_ks_error(keystore2.getSecurityLevel(SecurityLevel::TRUSTED_ENVIRONMENT)).unwrap();
        let (key, cert, cert_chain) = make_ec_signing_key(&*sec_level, "keystore2_pruning_test_key").unwrap();
        assert!(cert.is_some());
        assert!(cert_chain.is_none());

        let _exclusive_ops = OP_LIMIT.get_exclusive(SecurityLevel::TRUSTED_ENVIRONMENT);

        let first_op = sec_level.createOperation(
            &key,
            &AuthSetBuilder::new()
                .purpose(KeyPurpose::SIGN)
                .digest(Digest::SHA_2_256),
        ...
        {
            assert!(challenge.is_none());
            assert_eq!(Err(Error::Km(ErrorCode::INVALID_OPERATION_HANDLE)), 
