...
interface ISecureElementSession {
     ...
     *
     * ## Error conditions
     * `SecureElementErrorCode::SESSION_CLOSED` if trying to open a channel on a closed session.
     * `SecureElementErrorCode::MISSING_LISTENER` if listener object is null.
     * `SecureElementErrorCode::UNSUPPORTED_P2_BYTE` if the P2 byte (which is
     *                           part of ISO 7816) is not supported.
     ...
     *
     * ## Error conditions
     * `SecureElementErrorCode::SESSION_CLOSED` if the session is closed.
     * `SecureElementErrorCode::MISSING_LISTENER` if listener object is null.
     * `SecureElementErrorCode::UNSUPPORTED_P2_BYTE` if the P2 byte (which is
     *                           part of ISO 7816) is not supported.
     * `SecureElementErrorCode::IO_ERROR` if there was an error communicating with
